...
| `json.JSONDecodeError: Invalid control character` beim Parsen von GitHub-API-Responses | `diff_hunk`-Feld in Pull Request Review Comments enthält rohen Diff-Text mit unescapten `\t`/`\n`/`\r` — RFC 8259-Verstoß, bekannt als [cli/cli #10474](https://github.com/cli/cli/issues/10474), noch nicht behoben | `json.load(fp, strict=False)` oder `json.loads(text, strict=False)` — parst auch RFC-8259-inkonforme Responses. Betrifft `/pulls/{nr}/comments`-Endpunkt immer wenn `diff_hunk` Code-Einrückungen enthält. |
